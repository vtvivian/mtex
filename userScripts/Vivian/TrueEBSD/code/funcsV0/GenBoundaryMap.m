...
for c=1:3
    ...
    if valmin<valmax %sometimes if you have a binary image valmin = valmax
        testimg_noisy(:,:,c)=(temp-valmin)/(valmax-valmin);
    elseif valmax>0 %try to just normalise
        testimg_noisy(:,:,c)=temp/valmax;
    else %but avoid dividing by zero
        testimg_noisy(:,:,c)=temp;
    end
end
